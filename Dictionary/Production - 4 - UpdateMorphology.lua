...
for llIndex=0,LangLanguage:GetNumEntries()-1,1 do
  ...
  if tFrameWindow():GetSectionWindow(CurrentEntry:GetParent()):IsTagged(CurrentEntry) then
    if tRequestModify(CurrentEntry, true) then
      if CurrentEntry:GetNumChildrenOfElementType(ElementMorphology:GetID()) == 0 then
        --create a morphology for each word
        tLuaLog("Creating Morphology Elements");
        local lemma = CurrentEntry:GetLemmaSign();
        local _, c = lemma:gsub(" ","");
        tLuaLog(c.." words found");
        for eIndex = 0, c, 1 do
          local NodeMorphology = Doc:AllocateElementByName("Morphology",true);
          CurrentEntry:AddChildOrdered(NodeMorphology);
        ...
        for eIndex=0, CurrentEntry:GetNumChildrenOfElementType(ElementMorphology:GetID())-1,1 do
          ...
          tLuaLog("Populating Morphology String");
      ...
      if ElementLemmas == nil then
        ...
        if CurrentEntry:GetNumChildrenOfElementType(ElementLemmas:GetID()) == 0 then
          --create a morphology for each word
          local lemma = CurrentEntry:GetLemmaSign();
          local _, c = lemma:gsub(" ","");
          tLuaLog(c.." words found");
          for eIndex = 0, c, 1 do
            ...
            CurrentEntry:AddChildOrdered(NodeLemmas);
            tLuaLog("adding morphology")
          end
        else
          for eIndex=0, CurrentEntry:GetNumChildrenOfElementType(ElementLemmas:GetID())-1,1 do
            local Lemmas = CurrentEntry:GetNthChildOfElementType(ElementLemmas:GetID(),eIndex);
