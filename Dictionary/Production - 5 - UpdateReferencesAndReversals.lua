...
--Elliot Thornton
  ...
  function CopyExPhraseToElement(Entry,Recipient)
    tLuaLog("CopyExPhraseToElement");
    for eIndex=0,Entry:GetNumChildrenOfElementType(ElementExPhrase:GetID())-1,1 do
      local ExPhrase = Entry:GetNthChildOfElementType(ElementExPhrase:GetID(),eIndex);
      recursiveClone(Recipient,ExPhrase);
    end 
  end
  function CopyExampleToElement(Entry,Recipient)
    tLuaLog("CopyExampleToElement");
    for eIndex=0,Entry:GetNumChildrenOfElementType(ElementExample:GetID())-1,1 do
      local Example = Entry:GetNthChildOfElementType(ElementExample:GetID(),eIndex);
      recursiveClone(Recipient,Example);
    end 
  end

  function CopyHistoricalFormToElement(Entry,Recipient)
    tLuaLog("CopyHistoricalFormToElement");
    for eIndex=0,Entry:GetNumChildrenOfElementType(ElementHistoricalForm:GetID())-1,1 do
      local HistoricalForm = Entry:GetNthChildOfElementType(ElementHistoricalForm:GetID(),eIndex);
      recursiveClone(Recipient,HistoricalForm);
    end 
  end
  ...
  local eIndex = 0;
  local seIndex = 0;
  for llIndex=0,LangLanguage:GetNumEntries()-1,1 do
    local CurrentEntry = LangLanguage:GetEntry(llIndex);
    if tFrameWindow():GetSectionWindow(CurrentEntry:GetParent()):IsTagged(CurrentEntry) then
      --THIS POPULATES REVERSALS CONNECTED TO CurrentEntry OR UPDATES CURRENT ENTRY IF CurrentEntry IS A REVERSAL
      if tRequestModify(CurrentEntry, true) then
        tLuaLog("checkout out "..CurrentEntry:GetLemmaSign());
        local currentSection = CurrentEntry:GetParent();
        ...
        if currentSection == Doc:GetDictionary():GetLanguage(1) then
          ...
          local eIndexFix = 0;
          for eIndex=0, CurrentEntry:GetNumChildren()-1,1 do
          --walk through each reversalsense
            local elementName = CurrentEntry:GetChild(eIndex-eIndexFix):GetElement():GetName();
            if elementName == "ReversalSense" then
            --Follow reference to the Sense/Def
            --Continue with populating based on State
              local RevSense = CurrentEntry:GetChild(eIndex-eIndexFix);
              ...
              if RevSense:GetNumChildrenOfElementType(ElementReferences:GetID()) > 0 then
                ...
                if ref:GetNumRefEntries() > 0 then
                  ...
                  if RefSenseDef == nil then
                    ...
                    local TE = MatchTEWithinSense(CurrentEntry:GetLemmaSign(),RefSenseDef);
                    if TE ~= nil then
            ...
            if elementName == "Subentry" then
              ...
              local Subentry = CurrentEntry:GetChild(eIndex);
              local seIndexFix = 0;
              for seIndex=0, Subentry:GetNumChildren()-1,1 do
                --walk through each reversalsense
                  local elementName = CurrentEntry:GetChild(seIndex-seIndexFix):GetElement():GetName();
                if elementName == "ReversalSense" then
                --Follow reference to the Sense/Def
                --Continue with populating based on State
                  local RevSense = CurrentEntry:GetChild(seIndex-seIndexFix);
                  if RevSense:GetNumChildrenOfElementType(ElementReferences:GetID()) > 0 then
                    local ref = tolua.cast(RevSense:GetNthChildOfElementType(ElementReferences:GetID(),0),"tcReferences");
                    if ref:GetNumRefEntries() > 0 then
                      local RefSenseDef = ref:GetRefSense(ref:GetRefEntryID(0),0);
        ...
        for eIndex=0, CurrentEntry:GetNumChildrenOfElementType(ElementTE:GetID())-1,1 do
          local TE = CurrentEntry:GetNthChildOfElementType(ElementTE:GetID(),eIndex-eIndexFix);
        ...
        for eIndex=0, CurrentEntry:GetNumChildren()-1,1 do
          local elementName = CurrentEntry:GetChild(eIndex-eIndexFix):GetElement():GetName();
            if elementName == "Sense" then
              local Sense = CurrentEntry:GetChild(eIndex-eIndexFix);
              sIndexFix = 0;
          ...
          if elementName == "Subentry" then
            local Subentry = CurrentEntry:GetChild(eIndex-eIndexFix);
            for seIndex = 0, Subentry:GetNumChildren()-1,1 do
      ...
      tLuaLog("checking out "..CurrentEntry:GetLemmaSign());
      if tRequestModify(CurrentEntry, true) then
          for rIndex=0, CurrentEntry:GetNumDescendantsOfElementType(ElementReferences:GetID())-1,1 do
              local refNode = CurrentEntry:GetNthDescendantOfElementType(ElementReferences:GetID(), rIndex);
      ...
      local targetID = CurrentEntry:GetID();
      local refTable = {};
      for sIndex=0, Doc:GetDictionary():GetNumSections()-1,1 do
          ...
          for lIndex=0,Section:GetNumEntries()-1,1 do
              ...
              for eIndex=0, Entry:GetNumDescendantsOfElementType(ElementReferences:GetID())-1,1 do
                  ...
                  if References == nil then
                    tLuaLog("FAILED on: "..Entry:GetLemmaSign().." - Reference# "..eIndex);
                  else
